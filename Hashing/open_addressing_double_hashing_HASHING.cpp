...
int hash_function_2(string& name)
  {
  unsigned long int hash = 0;
  for(int i = 0;i < name.length();i++)
    hash += (pow(i,11)+name.at(i));
  //cout<<"HASH 2:"<<hash<<endl;
  return (hash%MAX_LENGTH);
  }
int next_index(int index,int counter,string& name)
  {
  int next_i = index + pow(counter,2) + counter * hash_function_2(name);
  return (next_i%MAX_LENGTH);
  }
public:
hash_table()
  {
...
void add(string s,X d)
  ...
  while(hash_slot[index].name.size() != 0 && hash_slot[index].name != "DELETED")
    ...
    counter++;
    index = next_index(original_index,counter,s);
    }
...
int search(string& n)
  ...
  if(hash_slot[index].name == n)
    return index;
  counter++;
  index = next_index(original_index,counter,n);
  }while(hash_slot[index].name != "" /*&& index != original_index*/);//here i have commented "index != original_index" because it      will encounter a NULL point before touching original index also it's possible that it never touches original index again due to     more large and complex hash function but as "load factor is less than 0.5 always" so there are more empty spots to encounter the probability of empty slot is much much higher than original index to be again occur.
  return -1;
  }
public:
